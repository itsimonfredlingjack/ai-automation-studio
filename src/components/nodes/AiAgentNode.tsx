...
const PROVIDERS = [
  ...
  { value: "anthropic", label: "Anthropic", defaultModel: "claude-sonnet-4-20250514", defaultUrl: "https://api.anthropic.com" },
  { value: "ollama", label: "Ollama", defaultModel: "llama3", defaultUrl: "http://localhost:11434" },
] as const;
...
function AiAgentNodeComponent({ data, id }: NodeProps<AiAgentNodeData>) {
  ...
  const onProviderChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      const p = PROVIDERS.find((pr) => pr.value === e.target.value) ?? PROVIDERS[0];
      updateNodeData(id, {
        provider: p.value,
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 shadow-sm min-w-[300px]">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-card-foreground">AI Agent</span>
      </div>
